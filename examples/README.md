...
and no test runs them — a `prompt` step needs provider credentials, so CI never invokes one.
...
| [`jira-workflow.workflow.json`](jira-workflow.workflow.json) | A Jira-shaped flow whose steps are all `person-activity`, for trying person-activity steps and `goto` round trips without a real integration. |
