/**
 * A minimal JSON tokenizer for the run inspector's I/O panel: it splits an already pretty-printed JSON
 * document into spans the view colours. It only *splits* — every token's text is a slice of the input,
 * so re-joining the tokens reproduces the document byte for byte and nothing on screen is the
 * tokenizer's invention. Not a parser: the input is `JSON.stringify` output, already known-valid, and
 * the panel needs colour, not structure. Presentation, so the Designer authors its own (ADR 0025).
 */
 ...
 * carries, CONTEXT.md §Secret) is never mistaken for a key separator. The trailing `\s*:` group is what
 * promotes a string to a key; it is emitted as plain text so the join stays lossless.
 */
