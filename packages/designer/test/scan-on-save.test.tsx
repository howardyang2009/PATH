...
describe("useScanOnSave", () => {
  ...
  it("lands its scan under StrictMode", async () => {
    ...
    const { result } = renderHook(() => useScanOnSave(fetch, "idle", RESCAN_ON), { wrapper: strict });
  ...
  it("re-scans when a named save phase lands, and not on others", async () => {
    ...
    const { result, rerender } = renderHook(({ phase }: { phase: SaveState["phase"] }) => useScanOnSave(fetch, phase, RESCAN_ON), {
  ...
  it("keeps the last good value beside a failure", async () => {
    let fail = false;
    const fetch = vi.fn(() => (fail ? Promise.reject(new Error("offline")) : Promise.resolve(["kept"])));
    const { result, rerender } = renderHook(({ phase }: { phase: SaveState["phase"] }) => useScanOnSave(fetch, phase, RESCAN_ON), {
      initialProps: { phase: "idle" as SaveState["phase"] },
    });
    ...
    fail = true;
    rerender({ phase: "saved" });
    await waitFor(() => expect(result.current).toEqual({ phase: "error", message: "offline", lastGood: ["kept"] }));
  });
});
