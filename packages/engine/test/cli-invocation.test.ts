...
describe("parseRunInvocation — the compatibility refusals", () => {
  const refusals: readonly (readonly [string, string[], RegExp])[] = [
    ...
    [
      "--list-eligible requires --resume",
      ["wf.json", "--list-eligible"],
      /--list-eligible requires --resume/,
    ],
    [
      "--list-eligible excludes --from",
      ["wf.json", "--resume", "r", "--list-eligible", "--from", "k"],
      /cannot be combined with --from/,
    ],
    [
      "--list-eligible launches nothing",
      ["wf.json", "--resume", "r", "--list-eligible", "--config", "c.json"],
      /cannot be combined with --config/,
    ],
    [
      "a resume restores context",
      ["wf.json", "--resume", "r", "--context", "c.json"],
      /cannot be combined with --resume/,
    ],
    [
      "a resume fixes the worker-default",
      ["wf.json", "--resume", "r", "--worker-default", "prompt=sdk"],
      /--worker-default cannot be combined with --resume/,
    ],
  ];
