...
describe("evaluateCondition — leaf predicates over strict semantics (ticket #21)", () => {
  it("exists is true when the path resolves and false (not error) when it does not", () => {
    ...
    expect(missing.trace).toMatchObject({
      type: "exists",
      path: "context.absent",
      outcome: "false",
    });
  ...
  it("one-of is true when the value is in the set", () => {
    expect(
      evalCond({ type: "one-of", path: "context.name", values: ["bob", "alice"] }).outcome,
    ).toBe("true");
    expect(evalCond({ type: "one-of", path: "context.name", values: ["bob"] }).outcome).toBe(
      "false",
    );
  });

  it("matches tests a regex against a string value", () => {
    expect(evalCond({ type: "matches", path: "context.name", pattern: "^al" }).outcome).toBe(
      "true",
    );
    expect(evalCond({ type: "matches", path: "context.name", pattern: "^zz" }).outcome).toBe(
      "false",
    );
  });

  it("range honors min and max bounds", () => {
    expect(evalCond({ type: "range", path: "context.count", min: 1, max: 10 }).outcome).toBe(
      "true",
    );
...
describe("evaluateCondition — strict error semantics (ticket #21)", () => {
  ...
  it("a type mismatch is an error (matches on a non-string, range on a non-number)", () => {
    expect(evalCond({ type: "matches", path: "context.count", pattern: "5" }).outcome).toBe(
      "error",
    );
...
describe("evaluateCondition — combinators (ticket #21)", () => {
  ...
  it("any is true when at least one child is true", () => {
    expect(
      evalCond({
        type: "any",
        of: [
          { type: "equals", path: "context.count", value: 99 },
          { type: "exists", path: "context.name" },
        ],
      }).outcome,
    ).toBe("true");
  ...
  it("not inverts a true/false child", () => {
    expect(evalCond({ type: "not", of: { type: "exists", path: "context.name" } }).outcome).toBe(
      "false",
    );
    expect(evalCond({ type: "not", of: { type: "exists", path: "context.absent" } }).outcome).toBe(
      "true",
    );
  });
  ...
  it("an error child dominates: all/any/not of an error are themselves error", () => {
    ...
    expect(
      evalCond({ type: "all", of: [{ type: "exists", path: "context.name" }, errChild] }).outcome,
    ).toBe("error");
    expect(
      evalCond({ type: "any", of: [{ type: "exists", path: "context.name" }, errChild] }).outcome,
    ).toBe("error");
