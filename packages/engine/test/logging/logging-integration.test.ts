...
describe("logging — end to end through runWorkflow (ticket #19)", () => {
  ...
  it("validates every run.log event against the event schema", async () => {
    ...
    await runWorkflow(stampNames(twoStepWorkflow), projectDir, {
      observer: composeObservers(createLoggingObserver(backends)),
    });
  ...
  it("records the failing step's step-finished with its error, then the failed root step-finished", async () => {
    ...
    const result = await runWorkflow(stampNames(failing), projectDir, {
      observer: composeObservers(createLoggingObserver(backends)),
    });
  ...
  it("narrates a parallel collect join in both backends, with branch ids and published keys (ticket #24)", async () => {
    ...
    const backends = createLogBackends(["db", "ndjson"], { db, projectDir });
    const result = await runWorkflow(stampNames(parallelWorkflow), projectDir, {
      observer: composeObservers(createLoggingObserver(backends)),
    });
    expect(result.status).toBe("succeeded");
  ...
  it("narrates a run-cancelled in both backends and marks the cancelled step's row cancelled (ticket #24)", async () => {
    ...
    const observer = composeObservers(
      createPersistedObserver(db, projectDir),
    ...
    expect(
      dbEvents.some(
        (e) => e.type === "step-finished" && e.node_id === "sleeper" && e.status === "cancelled",
      ),
    ).toBe(true);
  ...
  it("ends an externally aborted root run cancelled in both backends, the run rows and context.json (#52)", async () => {
    ...
    const result = await runWorkflow(stampNames(cancellableWorkflow), projectDir, {
      observer,
    ...
    expect(dbEvents.at(-1)).toMatchObject({
      type: "step-finished",
      run_id: root,
      node_id: null,
      status: "cancelled",
  ...
  it("lands no publish from a cancelled step in context.json, wherever the abort arrives (#52)", async () => {
    ...
    for (const delayMs of [0, 20, 70, 130, 200, 400]) {
      ...
      const result = await runWorkflow(stampNames(file), projectDir, {
        observer,
        signal: controller.signal,
      });
      clearTimeout(timer);
      ...
      const succeeded = rows
        .filter((r) => r.nodeId !== null && r.status === "succeeded")
        .map((r) => r.nodeId);
      const context = JSON.parse(
        readFileSync(join(runBlobDir(projectDir, rootId, rootId), "context.json"), "utf8"),
      );
  ...
  it("selects backends by the log.backends setting — 'none' produces no audit stream", async () => {
    ...
    const result = await runWorkflow(stampNames(twoStepWorkflow), projectDir, {
      observer: composeObservers(createLoggingObserver(backends)),
    });
    expect(result.status).toBe("succeeded");
  ...
  it("emits checkpoint and branch control events with complete traces to both backends (ticket #21)", async () => {
      ...
      body: [
        ...
        },
        {
          ...
          arms: [
            ...
            {
              when: { type: "equals", path: "context.pick", value: "b" },
              node: {
                type: "binary",
                id: "arm-b",
                name: "arm-b",
    ...
    const backends = createLogBackends(["db", "ndjson"], { db, projectDir });
    const result = await runWorkflow(stampNames(controls), projectDir, {
      observer: composeObservers(createLoggingObserver(backends)),
    });
    expect(result.status).toBe("succeeded");
    ...
    expect(checkpoint).toMatchObject({
      node_id: "gate",
      trace: { type: "exists", path: "context.pick", outcome: "true" },
    });
    ...
    expect(branch).toMatchObject({
      node_id: "route",
      arm: 1,
      trace: { type: "equals", path: "context.pick", outcome: "true" },
    });
  ...
  it("emits while-do iteration-started and loop-exited control events with traces to both backends (ticket #23)", async () => {
      ...
      body: [
        ...
        },
        {
          ...
          node: {
            ...
            args: [
              "-e",
              "let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>process.stdout.write(String(Number(d)+1)))",
            ],
            parse: "json",
            publish: { count: "${output}" },
          },
    ...
    const backends = createLogBackends(["db", "ndjson"], { db, projectDir });
    const result = await runWorkflow(stampNames(loop), projectDir, {
      observer: composeObservers(createLoggingObserver(backends)),
    });
    expect(result.status).toBe("succeeded");
    ...
    expect(exited).toMatchObject({
      ...
      iterations: 2,
      trace: { type: "range", path: "context.count", outcome: "false" },
    });
