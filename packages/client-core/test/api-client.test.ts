...
describe("PathApiClient", () => {
  ...
  it("resumeRun sends { rerun_from_run_id } as a JSON body for a Resume-from-K, config omitted", async () => {
    const inits: (RequestInit | undefined)[] = [];
    const stub = stubFetch((_url, init) => {
      inits.push(init);
      return json({ run_id: "successor", root_run_id: "successor" }, 202);
    ...
    await client.resumeRun("r1", undefined, "k-run-id");
    expect(inits[0]?.method).toBe("POST");
    expect(inits[0]?.body).toBe(JSON.stringify({ rerun_from_run_id: "k-run-id" }));
    expect(inits[0]?.headers).toMatchObject({ "Content-Type": "application/json" });
  });

  it("resumeRun sends both config and rerun_from_run_id when a K carries an override", async () => {
    const inits: (RequestInit | undefined)[] = [];
    const stub = stubFetch((_url, init) => {
      inits.push(init);
      return json({ run_id: "successor", root_run_id: "successor" }, 202);
