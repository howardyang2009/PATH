...
/**
 * `GET /v0/runs/:root_run_id/blobs/:run_id/:name` (name ∈ {input, output, context}) — returns the
 * run's on-disk `input.json`/`output.json`/`context.json` blob, already secret-masked at the
 * persistence boundary (CONTEXT.md §Secret), as `application/json`. Closes the gap map #29 flagged:
 * `input_ref`/`output_ref` are server-local FS paths a browser client can't read directly.
 *
 * 404 when the root/run is unknown, the name isn't a served blob, or the blob file is absent. Only a
