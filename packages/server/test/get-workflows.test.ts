...
function workflow(name: string, refs: string[] = []): string {
  ...
  return JSON.stringify({ format: "path/workflow@4", id: uuid(), name, body });
}
...
describe("GET /v0/workflows", () => {
  ...
  it("reports a superseded @1 file as invalid, with the codemod named in its error", async () => {
    ...
    expect(old.error?.message).toBe(
      `${join(projectDir, "old.workflow.json")}: path/workflow@1 is no longer read — run scripts/migrate-workflow-format-v2.ts then scripts/migrate-workflow-format-v3.ts then scripts/migrate-workflow-format-v4.ts to migrate this file to path/workflow@4`,
    );
