...
const ACCEPTANCE_DIR = fileURLToPath(
  new URL("../../../docs/acceptance-workflow/", import.meta.url),
);
...
describe("real acceptance workflow files validate clean", () => {
  it("validates release-notes.workflow.json", () => {
    ...
    if (!result.success) {
      throw new Error(
        `release-notes.workflow.json failed to validate:\n${result.errors.join("\n")}`,
      );
    }
  ...
  it("validates revise-cycle.workflow.json", () => {
    ...
    if (!result.success) {
      throw new Error(
        `revise-cycle.workflow.json failed to validate:\n${result.errors.join("\n")}`,
      );
    }
    expect(result.success).toBe(true);
  });
  ...
  it("validates do-not-wait-probe.workflow.json", () => {
    const result = safeParseWorkflowFile(
      loadJson("do-not-wait-probe.workflow.json"),
      builtinRegistry,
    );
    if (!result.success) {
      throw new Error(
        `do-not-wait-probe.workflow.json failed to validate:\n${result.errors.join("\n")}`,
      );
    }
    expect(result.success).toBe(true);
  });
